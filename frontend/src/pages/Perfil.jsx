...
export default function Perfil() {
  ...
  const cargarDatos = async () => {
    try {
      ...
      setFormData({
        ...
        objetivo: resPerfil.data.objetivo || '',
        nivel: resPerfil.data.nivel || '',
        diasEntreno: resPerfil.data.diasEntreno || ''
      });
  ...
  const handleGuardar = async (e) => {
    ...
    setGuardando(true);
    try {
      const res = await api.put('/perfil/editar', formData);
      setPerfil(res.data);
      setEditando(false);
  ...
  if (!perfil) return <div className="min-h-screen bg-gray-900 text-white flex justify-center items-center text-xl font-bold">Cargando perfil...</div>;
          ...
          <div className="flex flex-col gap-4">
            <div className="flex items-center justify-between border-b border-gray-700 pb-2">
              ...
              <button
                ...
                className={`text-sm px-4 py-2 rounded-lg text-white font-bold transition-colors ${editando ? 'bg-gray-600 hover:bg-gray-500' : 'bg-red-600 hover:bg-red-700 shadow-md shadow-red-600/30'}`}
              >
                {editando ? 'Cancelar' : '✏️ Editar Datos'}
              </button>
            </div>

            <div className="bg-gray-800 p-6 md:p-8 rounded-3xl border border-gray-700 shadow-2xl">
              {editando ? (
                <form onSubmit={handleGuardar} className="flex flex-col gap-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-gray-400 text-xs font-bold mb-1 uppercase tracking-wider">Peso (kg)</label>
                      <input type="number" step="0.1" name="peso" value={formData.peso} onChange={handleChange} className="w-full bg-gray-900 text-white rounded-xl p-3 border border-gray-700 focus:border-red-500 focus:outline-none focus:ring-1 focus:ring-red-500 transition-all" />
                    </div>
                    <div>
                      <label className="block text-gray-400 text-xs font-bold mb-1 uppercase tracking-wider">Altura (m)</label>
                      <input type="number" step="0.01" name="altura" value={formData.altura} onChange={handleChange} className="w-full bg-gray-900 text-white rounded-xl p-3 border border-gray-700 focus:border-red-500 focus:outline-none focus:ring-1 focus:ring-red-500 transition-all" />
                    </div>
                  </div>

                  <div>
                    <label className="block text-gray-400 text-xs font-bold mb-1 uppercase tracking-wider">Objetivo Físico</label>
                    <select name="objetivo" value={formData.objetivo} onChange={handleChange} className="w-full bg-gray-900 text-white rounded-xl p-3 border border-gray-700 focus:border-red-500 focus:outline-none cursor-pointer">
                  ...
                  <div>
                    ...
                    <select name="nivel" value={formData.nivel} onChange={handleChange} className="w-full bg-gray-900 text-white rounded-xl p-3 border border-gray-700 focus:border-red-500 focus:outline-none cursor-pointer">
                  ...
                  <div>
                    ...
                    <input type="number" min="1" max="7" name="diasEntreno" value={formData.diasEntreno} onChange={handleChange} className="w-full bg-gray-900 text-white rounded-xl p-3 border border-gray-700 focus:border-red-500 focus:outline-none" />
                  </div>

                  <button type="submit" disabled={guardando} className="mt-4 bg-gradient-to-r from-red-600 to-red-500 hover:from-red-500 hover:to-red-400 text-white font-black py-4 rounded-xl transition-all shadow-lg shadow-red-600/30 transform hover:-translate-y-1">
                ...
                <div className="flex flex-col gap-6">
                  <div className="flex items-center gap-4 mb-4">
                    <div className="w-20 h-20 bg-gray-700 rounded-full flex items-center justify-center text-3xl border-2 border-red-500 shadow-lg shadow-red-500/20">
                      👤
                    </div>
                    <div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-gray-900 p-4 rounded-2xl border border-gray-700 flex flex-col items-center justify-center">
                  ...
                  <div className="flex flex-col gap-3 text-sm mt-2">
                    <div className="flex justify-between border-b border-gray-700 pb-2">
                      ...
                      <span className="font-bold text-red-400">{perfil.objetivo.replace('_', ' ')}</span>
                    </div>
                    <div className="flex justify-between border-b border-gray-700 pb-2">
                      <span className="text-gray-400 uppercase tracking-wider font-bold text-xs">Nivel</span>
                      <span className="font-bold text-white">{perfil.nivel}</span>
                    </div>
                    <div className="flex justify-between border-b border-gray-700 pb-2">
          ...
          <div className="flex flex-col gap-4">
            <div className="flex items-center justify-between border-b border-gray-700 pb-2">
              ...
              <span className="bg-red-600/20 text-red-500 font-bold px-3 py-1 rounded-full text-sm">
                {historial.length} Entrenamientos
              </span>
            </div>

            <div className="bg-gray-800 p-6 rounded-3xl border border-gray-700 shadow-2xl min-h-[400px] flex flex-col relative overflow-hidden">
              ...
              {historial.length > 0 ? (
                <div className="flex flex-col gap-3 overflow-y-auto max-h-[600px] pr-2 custom-scrollbar mt-2">
                  {historial.map((entreno) => (
                    <div key={entreno.id} className="flex justify-between items-center bg-gray-900 p-4 rounded-2xl border border-gray-700 hover:border-gray-500 transition-colors">
                      <div className="flex items-center gap-4">
                        <div className="w-12 h-12 bg-gray-800 rounded-full flex items-center justify-center text-xl shadow-inner border border-gray-700">
                          🏋️‍♂️
                        </div>
                        <div>
